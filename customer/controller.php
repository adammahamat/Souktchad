...
function doInsert(){
    ...
    if(isset($_POST['submit'])){
        ...
        if(!isset($_POST['proid'])){
            echo "<script> alert('Felicitations, votre enregistrement a passe avec succees.il sera redirige vers votre profil.'); </script>";
...
function doEdit(){
    if(isset($_POST['save'])){
        ...
        message("Le compte a ete mis a jours!", "success");
        redirect(web_root.'index.php?q=profile');
    }
}
...
function doDelete(){
    ...
    if(isset($_SESSION['U_ROLE'])=='Customer'){
        ...
        if (isset($_POST['selector'])==''){
            message("Selectionnez d'abord les enregistrements avant de les supprime!","error");
            ...
            for($i=0;$i<$key;$i++){
                ...
                message("La commande a ete supprimer!","info");
        ...
        if (isset($_POST['selector'])==''){
            ...
            for($i=0;$i<$key;$i++){
                ...
                message("Le client a ete supprime!","info");
                redirect('index.php');
...
function processorder(){
    ...
    for ($i=0; $i < $count_cart  ; $i++) {
        ...
        $summary->ORDEREDREMARKS 	= 'Votre commande est en progres.';
        $summary->HVIEW 			= 0	;
        $summary->create();
    }
    ...
    message("La commande a ete cree avec succes!", "success");
    redirect(web_root."index.php?q=profile");
...
function processwishlist(){
    ...
    if(isset($_GET['wishid'])){
        ...
        if (isset($res)){
            message("Le produit a ete ajouter dans votre liste de favoris", "success");
            redirect(web_root."index.php?q=profile");
        }
...
function addwishlist(){
    ...
    if($maxrow>0){
        message("Le produit a ete deja ajouter dans votre liste de favoris", "error");
        redirect(web_root."index.php?q=profile");
    }else{
        $query ="INSERT INTO `tblwishlist` (`PROID`, `CUSID`, `WISHDATE`, `WISHSTATS`)  VALUES ('{$proid}','{$id}','".DATE('Y-m-d')."',0)";
        $mydb->setQuery($query);
...
function doupdateimage(){
    ...
    if ( $errofile > 0) {
        message("Aucone image selectionnee!", "error");
        redirect(web_root. "index.php?q=profile");
    }else{
        ...
        if ($image_size==FALSE ) {
            message(web_root. "Le ficher telecharger n'est pas une image!", "error");
            redirect(web_root. "index.php?q=profile");
        }else{
            //uploading the file
...
function doChangePassword(){
    if (isset($_POST['save'])) {
        ...
        message("Le mot de passe a ete change!", "success");
        redirect(web_root.'index.php?q=profile');
    }
}
